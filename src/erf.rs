...
const SIGN_MASK: u64 = 9223372036854775808;
...
pub fn fabs(x: f64) -> f64 {
    ...
    f64::from_bits(x.to_bits() & abs_mask)
}
...
fn erfc2(ix: u32, mut x: f64) -> f64 {
    ...
    f64::exp(-z * z - 0.5625) * f64::exp((z - x) * (z + x) + r / big_s) / x
}
...
pub fn erf_cpp(x: f64) -> f64 {
    ...
    if t > 0.927734375f64 {
        ...
        let mut r = fma(-1.72853470e-5f64, t, 3.83197126e-4f64); // -0x1.220000p-16,0x1.91cfb2p-12
        let u = fma(-3.88396438e-3f64, t, 2.42546219e-2f64); // -0x1.fd1438p-9, 0x1.8d6342p-6
        r = fma(r, s, u);
        r = fma(r, t, -1.06777877e-1f64); // -0x1.b55cb8p-4
        r = fma(r, t, -6.34846687e-1f64); // -0x1.450aa0p-1
        r = fma(r, t, -1.28717512e-1f64); // -0x1.079d0cp-3
        r = fma(r, t, -t);
        r = -r.exp_m1();
        r = r.copysign(a);
        r
        ...
        let mut r = -5.96761703e-4f64; // -0x1.38e000p-11
        r = fma(r, s, 4.99119423e-3f64); //  0x1.471a58p-8
        r = fma(r, s, -2.67681349e-2f64); // -0x1.b691b2p-6
        r = fma(r, s, 1.12819925e-1f64); //  0x1.ce1c44p-4
        r = fma(r, s, -3.76125336e-1f64); // -0x1.812700p-2
        r = fma(r, s, 1.28379166e-1f64); //  0x1.06eba8p-3
        r = fma(r, a, a);
        r
    }
}
...
/// This provides better accuracy than separate multiply and add operations
#[inline]
fn fma(a: f64, b: f64, c: f64) -> f64 {
    // Rust std library has mul_add which implements FMA if hardware supports it
    a.mul_add(b, c)
}
...
mod tests {
    ...
    #[test]
    fn test_erf_cpp() {
        assert!(erf_cpp(f64::NAN).is_nan());
        assert_eq!(erf_cpp(f64::INFINITY), 1.0);
        assert_eq!(erf_cpp(f64::NEG_INFINITY), -1.0);
        assert_eq!(erf_cpp(0.0), 0.0);
        assert_eq!(erf_cpp(-0.0), -0.0);
        assert_eq!(erf_cpp(2.0), 0.9953222650189527);
    }
}
